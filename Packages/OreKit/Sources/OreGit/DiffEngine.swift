...
public actor DiffEngine {
    ...
    public func diffAgainstBase(
        ...
        // `...` compares against the merge base, so commits landing on the base
        // branch while the agent works don't show up as this workspace's changes.
        let mergeBase = (try? await git.run(
            ["merge-base", "HEAD", baseBranch], in: worktree
        ).trimmedStandardOutput) ?? baseBranch
