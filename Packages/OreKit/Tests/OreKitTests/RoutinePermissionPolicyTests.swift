...
struct RoutinePermissionPolicyTests {
    ...
    @Test func toolsThatExecuteOrWriteFromTheirArgumentsStillAsk() {
        for command in [
            ...
            "sort -o.git/hooks/pre-commit Sources/payload.txt",
            "sort --output=.git/hooks/pre-commit Sources/payload.txt",
            "sort -ro .git/hooks/pre-commit Sources/payload.txt",
            "tee /etc/hosts",
            "git log --format=%B -1 --output=.git/hooks/pre-commit",
            "git diff --output=../../outside.txt",
            "git show HEAD --output ../outside.txt",
            "git grep --open-files-in-pager=id TODO",
            "git grep -Oid TODO",
            "git grep -inO TODO",
            "tar -tf Sources/a.tar --checkpoint=1 --checkpoint-action=exec=id",
            "tar -tf Sources/a.tar --to-command=id",
            "uniq Sources/names.txt .git/hooks/pre-commit",
            "xxd -r Sources/payload.hex Sources/App.swift",
    ...
    @Test func ordinaryUsesOfThoseToolsRemainRoutine() {
        for command in [
            ...
            "sort -r Sources/names.txt",
            "git log --oneline -5",
            "git grep -n TODO Sources",
            "git grep -o TODO Sources",
            "tar -tf Sources/archive.tar",
            "uniq -c Sources/names.txt",
            "xxd Sources/App.swift",
            "env -u DEBUG swift test",
        ] {
            #expect(
                RoutinePermissionPolicy.isRoutine(command, workspacePath: workspace),
